...
pub fn detect(cursor_x: f64, cursor_y: f64, output_w: i32, output_h: i32) -> Option<(SnapZone, SnapRect)> {
    ...
    if top && left {
        return Some((SnapZone::TopLeftQuarter, SnapRect {
            x: 0, y: usable_y, w: usable_w / 2, h: usable_h / 2,
        }));
    }
    if top && right {
        return Some((SnapZone::TopRightQuarter, SnapRect {
            x: usable_w / 2, y: usable_y, w: usable_w / 2, h: usable_h / 2,
        }));
    }
    if bottom && left {
        return Some((SnapZone::BottomLeftQuarter, SnapRect {
            x: 0, y: usable_y + usable_h / 2, w: usable_w / 2, h: usable_h / 2,
        }));
    }
    if bottom && right {
        return Some((SnapZone::BottomRightQuarter, SnapRect {
            x: usable_w / 2, y: usable_y + usable_h / 2, w: usable_w / 2, h: usable_h / 2,
        }));
    }
    if top {
        return Some((SnapZone::Maximize, SnapRect {
            x: 0, y: usable_y, w: usable_w, h: usable_h,
        }));
    }
    if left {
        return Some((SnapZone::LeftHalf, SnapRect {
            x: 0, y: usable_y, w: usable_w / 2, h: usable_h,
        }));
    }
    if right {
        return Some((SnapZone::RightHalf, SnapRect {
            x: usable_w / 2, y: usable_y, w: usable_w / 2, h: usable_h,
        }));
    }
    None
}
