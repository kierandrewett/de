...
impl ShadowPass {
    pub fn new(shared: &Shared) -> Self {
        let silhouette_pipeline = make_pipeline(
            ...
            "shadow-silhouette", SHADER_SRC, "fs_silhouette",
            shared.format, None,
        );
        let blur_h_pipeline = make_pipeline(
            &shared.device, &shared.bgl,
            "shadow-blur-h", SHADER_SRC, "fs_blur_h",
            shared.format, None,
        );
        let blur_v_pipeline = make_pipeline(
            &shared.device, &shared.bgl,
            "shadow-blur-v", SHADER_SRC, "fs_blur_v",
            shared.format, None,
        );
        let composite_pipeline = make_pipeline(
            &shared.device, &shared.bgl,
            "shadow-composite", SHADER_SRC, "fs_composite",
    ...
    fn ensure_intermediates(&mut self, shared: &Shared, w: u32, h: u32) {
        if self.intermediates.as_ref().map_or(true, |i| i.width != w || i.height != h) {
            ...
            self.intermediates = Some(Intermediates {
                width: w, height: h,
                mask, mask_view, hblur, hblur_view, vblur, vblur_view,
            });
        }
    }
...
pub fn active_layers() -> [ShadowLayer; 3] {
    [
        ...
        ShadowLayer { offset_x: 0.0, offset_y: 0.0, blur_sigma: 1.0, alpha: 0.035 },
        // Main soft drop.
        ShadowLayer { offset_x: 0.0, offset_y: 3.0, blur_sigma: 6.0, alpha: 0.05  },
        // Slot kept for compatibility — alpha 0 so it's a no-op (focus_t
        // crossfade still uses 3 layer indices in render/mod.rs).
        ShadowLayer { offset_x: 0.0, offset_y: 0.0, blur_sigma: 1.0, alpha: 0.0   },
    ]
}
...
pub fn inactive_layers() -> [ShadowLayer; 2] {
    [
        ShadowLayer { offset_x: 0.0, offset_y: 0.0, blur_sigma: 1.0, alpha: 0.015 },
        ShadowLayer { offset_x: 0.0, offset_y: 2.0, blur_sigma: 4.0, alpha: 0.020 },
    ]
}
