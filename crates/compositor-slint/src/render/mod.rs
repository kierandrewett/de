...
pub struct WindowChromeParams {
    ...
    pub focus_t:   f32,
    pub mode_t:    f32,
    /// True for CSD apps. CSD windows still get the GPU drop-shadow but
    /// SKIP the 0.5 px border + 1 px top-edge highlight passes — those would
    /// draw a thin inset line over the client's own chrome and look wrong.
    pub csd:       bool,
}
...
impl ChromeRenderer {
    pub fn new(device: Arc<wgpu::Device>, format: wgpu::TextureFormat) -> Self {
        ...
        Self {
            shared, shadow, squircle_clip, border, highlight,
    ...
    pub fn render(
        ...
        for win in windows.iter() {
            ...
            for li in 0..2 {
                let a  = inactive[li].alpha
                       + win.focus_t * (active[li].alpha - inactive[li].alpha);
                ...
                let layer = shadow::ShadowLayer {
                    ...
                    offset_y: oy, blur_sigma: b, alpha: a,
                };
                let mut u = uniforms_for_layer(
                    win.x, win.y, win.w, win.h, sw, sh, layer, 1.0,
                );
                u.mode_t = win.mode_t;
            ...
            if win.focus_t > 0.001 {
                let layer3 = active[2];
                let mut u = uniforms_for_layer(
                    win.x, win.y, win.w, win.h, sw, sh, layer3, win.focus_t,
                );
                u.mode_t = win.mode_t;
                u.focus_t = win.focus_t;
                let slot = raw_uniforms.len() as u32;
            ...
            let clip_slot = if self.enable_squircle_clip {
                ...
                u.win_x = win.x; u.win_y = win.y; u.win_w = win.w; u.win_h = win.h;
                u.surface_w = sw; u.surface_h = sh;
                u.mode_t = win.mode_t; u.focus_t = win.focus_t;
                let slot = raw_uniforms.len() as u32;
                raw_uniforms.push(u);
                Some(slot)
            } else { None };
            ...
            per_win.push(PerWin { shadow_slots, clip_slot, border_slot, hl_slot });
        }
        ...
        for (wi, pw) in per_win.iter().enumerate() {
            ...
            for slot in &pw.shadow_slots {
                ...
                self.shadow.render_layer(
                    &self.shared, encoder, target_view,
                    surface_w, surface_h, dyn_offset,
                );
            }
            ...
            if let Some(slot) = pw.clip_slot {
                self.squircle_clip.render(
                    &self.shared, encoder, scene_view, target_view, slot_offset(slot),
                );
            }
            ...
            if !windows[wi].csd {
                self.border.render(
                    &self.shared, encoder, &dummy, target_view, slot_offset(pw.border_slot),
                );
                self.highlight.render(
                    &self.shared, encoder, &dummy, target_view, slot_offset(pw.hl_slot),
                );
            }
